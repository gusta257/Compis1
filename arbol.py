...
class Arbol:
    ...
    def crearHojasPipe(self, val1, val2, op):
        ...
        if(len(val1) == 1 and len(val2) == 1 and op =='|'):
            ...
            hoja3 = hojas.Hojas(id,'',op, [hoja1.get_id(),hoja2.get_id()] )
            self.nodos.append(hoja3)

            hoja1.set_padreID(hoja3.get_id())
            hoja2.set_padreID(hoja3.get_id())
            ...
            id+=1
            #(id, padreID, valor, hijos)
            hoja1 = hojas.Hojas(id,'',op,[hojaH1.get_id(),hojaH2.get_id()])
            hojaH2.set_padreID(hoja1.get_id())
            hojaH1.set_padreID(hoja1.get_id())
            self.nodos.append(hoja1)
    ...
    def crear_nodosStar(self,val1,op):
        ...
        if(len(val1) == 1 and op =='*'):
            ...
            hoja1.set_padreID(hoja2.get_id())
            hoja2.set_hijos(hoja1.get_id())
            ...
            hojaH.set_padreID(hoja1.get_id())
            hoja1.set_hijos(hojaH.get_id())
    ...
    def crear_nodosCat(self,val1,val2,op):
        ...
        if(len(val2)==1 and len(val1)>1):
            ...
            hojaH1.set_padreID(hoja2.get_id())

            hojaH2.set_padreID(hoja2.get_id())
            hijos = []
            hijos.append(hojaH1)
            hijos.append(hojaH2)
            for i in hijos:
                hoja2.set_hijos(i.get_id())
